              ...
              if status == 'НЕАКТИВИРОВАН'
                ...
                if user
                  ...
                  if check_birthday(user[:birthday])
                    ...
                    "Поздравляем, Вас с Днём рождения, Вам предоставляется на него скидка 30%\n" \
                    "🤩 #{barista_code[:code]}"
                    )
                    ...
                    bot.api.send_message(chat_id: message.chat.id, text: "✅ Вы активировали код. " \
                    "Приходите еще, как всегда рады Вас видеть ❤ \n\n" \
                    "Поздравляем, Вы заказали очередной 5-ый кофе, Вам предоставляется на него скидка 30%\n" \
                    "🤩 #{barista_code[:code]}"
                    )

                    bot.api.send_message(chat_id: barista_code[:user_telegram_id], text: "Клиенту #{user[:name]} #{user[:phone]} предоставляется скидка 30%. Код - #{barista_code[:code]}")
                  else
                    bot.api.send_message(chat_id: message.chat.id, text: "✅ Вы активировали код. " \
                    "Приходите еще, как всегда рады Вас видеть ❤ \n\n" \
